...
def wait_for_service(driver: Driver, max_retries: int = 5, delay: int = 5) -> bool:
    ...
    for i in range(max_retries):
        try:
            ...
            print("[OK] graph-guard service is ready")
            return True
            ...
            if i < max_retries - 1:
                ...
                print(f"[FAIL] Service failed to become ready: {e}")
                return False
    return False
...
def run_query(
    ...
    try:
        records: list[Any] = list(session.run(query, parameters or {}))
        count_note = ""
        if expected_count is not None and len(records) != expected_count:
            count_note = f" (expected {expected_count})"
        print(f"  [OK] {len(records)} record(s){count_note}")
...
def main(
    ...
    try:
        ...
        with driver.session() as session:
            section("MOVIES GRAPH TEST SUITE")

            section("SECTION 1: Match Queries (Read Operations)")
            ...
            section("SECTION 2: Graph Pattern Queries")
            ...
            section("SECTION 3: Aggregation & Analysis Queries")
            ...
            section("SECTION 4: Relationship Pattern Queries")
            ...
            section("SECTION 5: Write Operations")
            ...
            section("SECTION 6: Notification-Generating Queries (Performance Hints)")
            ...
            section("SECTION 7: Schema Validation Tests (Expected Failures)")
            ...
            section("SECTION 8: Cardinality Violation Queries (Verifier Violations)")
