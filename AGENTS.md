...
| `graph-guard-web`      | Elm + Vite frontend; communicates with the server via HTTP (`/api/*`) and WebSocket (`/ws`)   |

## Build Commands

- `./gradlew test` - run all tests
- `./gradlew :graph-guard:test` - run tests for a specific module
- `./gradlew spotlessApply` - format code (ktfmt + prettier + elm); must run detekt first
...
- `graph-guard-web` is a pure Elm + Vite project managed by the `com.github.node-gradle.node` Gradle
  plugin; it has no JVM source
- Elm source lives in `graph-guard-web/src/elm/` following the Elm Architecture: `Main.elm` (entry),
  `Model.elm` (types + decoders), `Update.elm` (state transitions), `View.elm` (rendering),
  `Ports.elm` (JS interop)
- `src/elm/Tailwind/` is generated by `elm-tailwind-modules`; don't edit it manually
- JS interop uses ports: outgoing commands (`initEditor`, `setEditorContent`, `getEditorContent`,
  `initSchemaView`, `copyToClipboard`) and incoming subscriptions (`wsMessage`, `wsStatus`,
  `editorChanged`) wired in `src/js/index.js`
- Run `npm run dev` inside `graph-guard-web/` for the local dev server (generates Tailwind modules
  then starts Vite)
